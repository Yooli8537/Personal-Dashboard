...
- Notes: A little notes section using the **TipTap** editor & JSON to save data automatically.
...
- App Status: Check the status of an app by adding its URL, which will run fetch calls / pings on it.
- GD Level Tracker: View all the levels you're currently progressing on. If none are being progressed, it shows the recommended level. Requires the [Geometry Dash Bucket List](https://github.com/Yooli8537/geometry-dash-bucket-list) to be active on an accessible network to the dashboard (Information in it's [README](https://github.com/Yooli8537/geometry-dash-bucket-list/#readme)).
...
- Settings
  ...
  - Light / Dark Mode
  - Extended Mode: Extra Options for widgets
- Styling: Make the dashboard actually look good.
- Error handling & communication (example: autosave icons)
